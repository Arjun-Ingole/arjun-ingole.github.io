...
for those who would still argue slapping an index would've fixed this issue as well, true, but indexes come with their own overhead. writes get a lil slower, storage use jumps, and it would've fixed the problem for a while. as the table grows, the problem would've resurfaced eventually, but the query times wouldn't immediately be in the ballpark of 40 seconds. they would've slowly crept up from 4 seconds to 8 seconds, so most of us would've just ended up blaming "the table is too big" and moving on
